...
export const getAllStores = async (
  ...
  try {
    ...
    if (req.query.province) searchQuery.province = req.query.province as string;
  ...
  try {
    ...
    addresses.forEach((address) => {
      ...
      if (address.country === "Canada") {
        ...
        if (
          !countryData.storeData.find((p) => p.province === address.province)
        ) {
          ...
          provData.stores = [
            {
              city: address.city,
              cityCount:
                cityCounts.find((c) => c.city === address.city)?.cityCount || 0,
            },
          ];
        ...
        if (!countryData.storeData.find((p) => p.state === address.state)) {
          const provData: Partial<StoreData> = {};
