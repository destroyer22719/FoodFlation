    ...
    if (!changePage) {
      storeReq = await fetch(
        `${
          ...
          provinceSearch ? `province=${provinceSearch}` : `state=${stateSearch}`
        }`
      );
      ...
      if (postalCode || zipCode)
        storeReq = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/stores?page=${page + pageInc}&${
            zipCode ? `zipCode=${zipCode}` : `postalCode=${postalCode}`
          }`
        );
      else
        storeReq = await fetch(
